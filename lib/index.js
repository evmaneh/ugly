...
module.exports = class {
  ...
  http(req, res, next = () => res.end("")) {
    ...
    var proxyURL = {
        ...
        origin: this.proxifyRequestURL(req.path, true)
          .split("/")
          .splice(0, 3)
          .join("/"),
        hostname: this.proxifyRequestURL(req.path, true)
          .split("/")
          .splice(0, 3)
          .slice(2)
          .join("/"),
    ...
    if (proxyOptions.headers["origin"]) {
      var proxified_header = this.proxifyRequestURL(
        `/${proxyOptions.headers["origin"].split("/").splice(3).join("/")}`.replace(
          this.prefix,
          "",
        ),
        true,
